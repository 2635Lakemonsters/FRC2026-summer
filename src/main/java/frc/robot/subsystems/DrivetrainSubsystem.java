...
public double toRedHead(double blueHeadingDegrees) { // Turn Angle from Blue to Red Alliance
  ...
  public void drive(
    ...
    applyModuleStates(swerveModuleStates);
  ...
  private void applyModuleStates(SwerveModuleState[] states) {
    m_frontLeft.setDesiredState(states[Constants.FRONT_LEFT_MODULE_STATE_INDEX]);
    m_frontRight.setDesiredState(states[Constants.FRONT_RIGHT_MODULE_STATE_INDEX]);
    m_backLeft.setDesiredState(states[Constants.BACK_LEFT_MODULE_STATE_INDEX]);
    m_backRight.setDesiredState(states[Constants.BACK_RIGHT_MODULE_STATE_INDEX]);
  }
